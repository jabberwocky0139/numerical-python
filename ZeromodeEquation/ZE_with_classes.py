...
class Procedures(metaclass=ABCMeta):
    ...
    def __MakeZeromodeHamiltonian(self, v, dmu):
        ...
        P2_a, P2_b = (v.I - 4 * v.D) / v.dq**2, -0.5 * (
            v.I - 4 * v.D) / v.dq**2
        ...
        QP2Q_a, QP2Q_b = 2 * v.C * (v.q - v.Nq / 2.0)**2, -v.C * (
            v.q - v.Nq / 2) * (v.q - v.Nq / 2 + 1)
...
class OutputZeromodeGroundFunction(Procedures):
    ...
    def Procedure(self, N):
        ...
        plt.plot(
            v.q, np.imag(val), label='iamginary part of zeromode function')

        plt.plot(v.q, np.sqrt(np.real(val)**2 + np.imag(val)**2), label='numerical')


        def psi(x, alpha):
            beta = 1 / (2 * np.sqrt(2) * alpha)
            gamma = -1 / (6 * np.sqrt(2) * alpha**3)
            return (1 / (2 * np.pi * alpha**2))**0.25 * np.exp(-x**2 / (
                4 * alpha**2) + 1j * (beta * x + gamma * x**3))

        # def psi_general(x, n, alpha):
        #     beta = 1 / (2 * np.sqrt(2) * alpha)
        #     gamma = -1 / (6 * np.sqrt(2) * alpha**3)
        #     C = np.sqrt(2**n * factorial(n) /
        #                 (np.sqrt(2 * np.pi) * factorial(2 * n) * alpha**
        #                  (2 * n + 1)))
        #     return C * x**n * np.exp(-x**2 / (4 * alpha**2) + 1j * (
        #         beta * x + gamma * x**3))
        #     # return C * eval_hermite(n, x/alpha) * np.exp(-x**2 / (4 * alpha**2) + 1j * (beta * x + gamma * x**3))

        # n = 0
        # numerator = np.sqrt(2) * (n + 1) * (4 * n**2 - 1) + np.sqrt(2 * (
        #     4 * n**2 - 1) * (36 * n**4 + 72 * n**3 + 59 * n**2 + 30 * n - 25))
        # dedominator = 8 * (4 * n**2 - 1) * (2 * n + 3)
        # alpha = (numerator/dedominator)**(1/3) * v.N**(-1/3)
        alpha = 0.707 * (v.N)**(-1/3)
        plt.plot(v.q, np.real(psi(v.q, alpha)),
                 '--', label='real part(variational)')
        plt.plot(v.q, np.imag(psi(v.q, alpha)),
                 '--', label='imaginary part(variational)')
        plt.plot(v.q, np.sqrt(psi(v.q, alpha)*np.conjugate(psi(v.q, alpha))),
                 '--', label='variational')
